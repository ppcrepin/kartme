...
| **2.6 Cycle de vie de course** | 🟢 **code prêt, à activer** *(construit 2026-07-14)* — **verrou optionnel** « Clôturer les invitations » → état `locked` (grille figée côté serveur via RLS) + **un rappel** push aux inscrits (sens unique, **pas de RSVP**, canal « Invitations »), **réversible** (« Rouvrir les invitations », rappel non renvoyé) ; l'usage spontané (saisie directe) reste possible. **Fenêtre de correction 24 h** *(version sûre restreinte)* : l'admin re-saisit le classement dans les 24 h avec recalcul Elo, **uniquement si aucun pilote n'a couru depuis** (sinon refus, pour ne pas fausser l'Elo « à chemin »). 6 scénarios de tests DB. Reviewer adversarial. *Reste : coller `20260714150000_race_lifecycle.sql` + test PO.* |
