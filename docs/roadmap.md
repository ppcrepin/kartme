...
- **Deux salles de Nancy sont posées sur le centre-ville, faute d'adresse** *(revue A16, 2026-07-30)* : Kart'Eam et Team Marius Karting partagent au mètre près les coordonnées de Nancy — le relevé ne les situe qu'à « Est de Nancy », sans adresse, donc le géocodage est retombé sur la commune (vérifié : Nominatim rend exactement ce point pour « Nancy »). Ce ne sont PAS des doublons — les noms n'ont aucun rapport — mais « Autour de toi » les annonce à la même distance. Une recherche par NOM chez Nominatim ne rend rien pour ces deux enseignes, et le PO n'a pas leurs adresses (2026-07-30) : la piste automatique est épuisée, il faudra les saisir à la main. Le test 99d plafonne ce compte, donc un futur import qui en ajoute échoue au lieu de passer inaperçu.
- *(Rectification 2026-07-30)* — **la paire BKI Brest / Speed Park n'est PAS un défaut** : elle avait d'abord été rangée avec le cas de Nancy. Vérification faite, « 10 rue Alain Le Berre, Brest » géocode exactement sur les coordonnées de BKI : sa position est à la RUE, donc juste. Ce sont deux établissements voisins de 317 m, comme les quatre paires à moins de 2 km déjà consignées ci-dessus.
